...
def forward():
    ...
    RIGHT_FWD.value(0)
    RIGHT_BWD.value(1)
    LEFT_PWM.duty_u16(pwm(SPEED_MOVE))
    RIGHT_PWM.duty_u16(pwm(SPEED_MOVE))
...
def run_repetition(target_angle, direction):
    ...
    while accumulated < target_angle and ticks_diff(ticks_ms(), turn_start) < timeout_ms:
        ...
        delta = gz * dt if direction == "E" else -gz * dt
        if delta > 0:
            accumulated += delta
    ...
    stop()
    sleep_ms(200)
    return accumulated, v_initial, v_min, current_initial, current_max
...
angle_index = choose_index("Angulo teste", ANGULOS)
target = ANGULOS[angle_index]

direction_index = choose_index("Direcao teste", [label for _, label in DIRECOES])
direction, direction_label = DIRECOES[direction_index]

filename = "val_{}_{}.csv".format(target, direction)
...
csv = open(filename, "a")
if new_file:
    csv.write("id,repeticao,angulo_alvo,angulo_acumulado,direcao,bateria_inicial,bateria_minima,corrente_inicial,corrente_maxima\n")
csv.flush()

for repetition in range(1, TOTAL_REPETICOES + 1):
    oled.msg("Rep {}/10".format(repetition), "{} {} graus".format(direction_label, target),
        "Joy inicia", "ID {}".format(experiment_id))
    wait_joy_press()

    angle_acc, v_initial, v_min, current_initial, current_max = run_repetition(target, direction)

    csv.write("{},{},{},{:.2f},{},{:.2f},{:.2f},{:.3f},{:.3f}\n".format(
        experiment_id, repetition, target, angle_acc, direction, v_initial, v_min,
        current_initial, current_max
    ))
    csv.flush()

    oled.msg("Rep {}/10 OK".format(repetition),
             "Ang {:.1f}".format(angle_acc),
             "Imax {:.2f}A".format(current_max),
             "Joy prox" if repetition < TOTAL_REPETICOES else "")

csv.close()
stop()

while True:
    oled.msg("CONCLUIDO 10/10", "Arquivo salvo:", filename[:16], "Reset p/ novo")
    sleep(1)
